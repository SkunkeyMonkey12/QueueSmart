...
# Load csv data
df = pd.read_csv("QueueSmartwork/Queuesmart data(in).csv", skiprows=1)
...
def calculate_ticket_priority(ticket):
    ...
    # Ticket age (1 per hour after 24 hours)
    created_str = ticket.get("Created Date", "")
    try:
    ...
    if staff_choice == "1":
        ...
        while (user_name in staff_usernames and
            ...
            # View tickets sorted by priority
                ...
                if tickets:
                    for t in tickets:
                        t["Priority_Score"] = calculate_ticket_priority(t)
                    tickets_sorted = sorted(
                        tickets, key=lambda x: x["Priority_Score"], reverse=True)
                    print("\n===== Tickets (sorted by priority) =====")
                    for i, t in enumerate(tickets_sorted, start=1):
                        print(f"\nTicket #{i}")
            ...
            # Weekly staff chart
                ...
                if appointments:
                    ...
                    plt.bar(staff_names, counts, color="red")
